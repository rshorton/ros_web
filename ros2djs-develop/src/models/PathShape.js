...
ROS2D.PathShape = function(options) {
	...
	if (path !== null && typeof path !== 'undefined') {
		this.graphics.setStrokeStyle(this.strokeSize, 'round', 'round', 1, true);
 ...
 */
ROS2D.PathShape.prototype.setPath = function(pathIn) {
	this.graphics.clear();
	if (pathIn !== null && typeof pathIn !== 'undefined') {
		this.graphics.setStrokeStyle(this.strokeSize, 'round', 'round', 1, true);
		this.graphics.beginStroke(this.strokeColor);
		this.graphics.moveTo(pathIn.poses[0].pose.position.x / this.scaleX, pathIn.poses[0].pose.position.y / -this.scaleY);
		for (var i=1; i<pathIn.poses.length; ++i) {
			this.graphics.lineTo(pathIn.poses[i].pose.position.x / this.scaleX, pathIn.poses[i].pose.position.y / -this.scaleY);
		}
		this.graphics.endStroke();
	}
};
